...
1. **測試檔案路徑**（必要）— 如 `tests/MyProject.AppHost.Tests/Integration/OrdersApiTests.cs`
...
1. 測試專案 `.csproj`
2. `AspireAppFixture.cs`
3. Collection Fixture 定義
4. IntegrationTestBase 基底類別
5. DatabaseManager（如有）
6. 所有 `*Tests.cs` 測試檔案

### Step 3：確認測試結果

如果 呼叫者 已提供 Executor 測試結果（全數通過），**跳過重新執行**。否則自行執行測試確認。
...
7. **覆蓋率以端點為單位**
8. **公正客觀**
9. **Resource 名稱交叉比對** — 比對 `CreateHttpClient` 與 `AddProject` 名稱
