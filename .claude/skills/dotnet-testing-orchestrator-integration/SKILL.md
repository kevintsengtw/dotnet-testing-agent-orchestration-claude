...
```bash
node .claude/scripts/dotnet-testing-claude-full/token_usage.js start integration 2>/dev/null
```
...
> 場景數多時 Writer 單次產出可能觸及輸出 token 上限，分兩次啟動是預設做法；單次可完成時不必強行分批。
...
1. 以 **Bash 工具**執行（此步只取得資料，使用者還看不到）：
   ...
   ```bash
   node .claude/scripts/dotnet-testing-claude-full/token_usage.js report integration 2>/dev/null
   ```
...
> **表格缺席時的判讀**：Token 表格缺席**不代表流程異常** —— 四階段的成敗一律以 Executor 回報與磁碟狀態為準。缺席只代表本次沒有 token 資料可看；transcript 仍在，使用者可自行執行 `node .claude/scripts/dotnet-testing-claude-full/token_usage.js report integration` 補取。**不得因表格缺席而重跑整個工作流程。**
