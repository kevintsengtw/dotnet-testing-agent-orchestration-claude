...
Executor 負責建置並執行整合測試，並帶有特殊的 Docker 前置檢查。
...
Executor **不修改 `src/`**。發現測試失敗的根因在生產程式碼時，保留失敗、記入 `productionObservations[]` 回報，由使用者決定。詳細規範見第 6 節。
...
整合測試是端對端驗證，測試失敗的根因可能在測試程式碼，也可能在生產程式碼本身。早期版本允許 Executor 直接修正後者，實測中出現過改動 DTO nullability 而使用者事前未被告知的情況——改動範圍由當輪判斷決定，使用者事後才發現。改為只回報後，要不要動、怎麼動由使用者決定。

**回報方式：**

發現的問題記入交接欄位 `productionObservations[]`，每筆包含：

| 欄位       | 內容                             |
| ---------- | -------------------------------- |
| `file`     | 檔案路徑                         |
| `location` | 類別／方法／行號等位置資訊       |
| `issue`    | 問題描述（**只描述，不修改**）   |
| `options[]`| 可能的處理方式                   |
...
生產程式碼問題導致的測試失敗**保留失敗、不修**，不以調整測試的方式迴避。Orchestrator 在結果整合時逐筆呈現觀察並等待使用者決定；使用者明確要求修改 `src/` 時，才走修改流程處理該項。
