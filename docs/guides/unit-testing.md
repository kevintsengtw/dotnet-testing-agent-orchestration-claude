...
本文件說明如何使用 `/dotnet-testing-orchestrator-unit` 指令，在 Claude Code 中自動完成 .NET 單元測試的分析、撰寫、執行與審查四個階段。
...
- **Claude Code CLI 已安裝**（`claude` 指令可在終端機執行）
- **dotnet-testing-agent-skills 已安裝**（`/dotnet-testing-orchestrator-unit` 斜線指令可在 Claude Code 工作階段中使用）
- **.NET SDK 8.0 / 9.0 / 10.0 至少一個版本**（`dotnet --version` 可確認）
- **不需要 Docker Desktop**（單元測試不使用容器，與整合測試不同）
...
在 Claude Code 工作階段中，輸入以下斜線指令觸發 Orchestrator：

```text
/dotnet-testing-orchestrator-unit
```

指令觸發後，提供目標類別的資訊給 Orchestrator，包含：

- 被測試目標的檔案路徑
- 測試專案路徑（`.csproj`）
- 簡短說明（可選，用於補充特殊需求）

Orchestrator 會依序自動啟動四個 subagent：Analyzer → Writer → Executor → Reviewer，全程無需手動介入。
...
```text
/dotnet-testing-orchestrator-unit 為 TemperatureConverter 撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/TemperatureConverter.cs
...
```text
/dotnet-testing-orchestrator-unit 為 WeatherAlertService 撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/Services/WeatherAlertService.cs
測試專案：samples/unit/practice/tests/Practice.Core.Tests/Practice.Core.Tests.csproj
```
...
```text
/dotnet-testing-orchestrator-unit 為 EmployeeService 撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/Services/EmployeeService.cs
測試專案：samples/unit/practice/tests/Practice.Core.Tests/Practice.Core.Tests.csproj
...
```text
/dotnet-testing-orchestrator-unit 為 ConfigurationLoader 的所有公開方法撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/Services/ConfigurationLoader.cs
測試專案：samples/unit/practice/tests/Practice.Core.Tests/Practice.Core.Tests.csproj
```
...
```text
/dotnet-testing-orchestrator-unit 為 OrderValidator 撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/Validators/OrderValidator.cs
測試專案：samples/unit/practice/tests/Practice.Core.Tests/Practice.Core.Tests.csproj
```
...
```text
/dotnet-testing-orchestrator-unit 為 OrderProcessingService、WeatherAlertService 撰寫單元測試。
被測試目標：samples/unit/practice/src/Practice.Core/Services/OrderProcessingService.cs, samples/unit/practice/src/Practice.Core/Services/WeatherAlertService.cs
測試專案：samples/unit/practice/tests/Practice.Core.Tests/Practice.Core.Tests.csproj
```

預期 Orchestrator 行為：

- Orchestrator 偵測到 2 個目標，平行啟動 2 個 Analyzer（同一回應中發出）
...
├── src/
│   └── Practice.Core/             # 待測試的應用程式碼（Orchestrator 的分析目標）
...
└── tests/
    └── Practice.Core.Tests/       # 空白測試專案目錄（由 Orchestrator 產生測試）
```

練習專案支援三個 .NET 版本，以子專案形式存在同一目錄內：`net8.0` / `net9.0` / `net10.0`。
...
Orchestrator 產生的測試檔案僅供練習使用，若要還原初始空白狀態：

```bash
git checkout -- samples/unit/practice/tests/
```

> **注意**：測試檔案不應 commit 到 repo。所有 `samples/*/tests/` 下產生的測試類別檔案與 `.csproj` 修改，請在練習完成後執行上述指令還原。
...
**症狀**：Orchestrator 委派 Writer 時，Writer 找不到 `dotnet-testing-autofixture-basics` 等技能，或斜線指令 `/dotnet-testing-orchestrator-unit` 無法使用。

**解法**：確認 `dotnet-testing-agent-skills` 外部 repo 已完成安裝。重新執行安裝指令：

```bash
npx skills install
```

安裝完成後重新啟動 Claude Code 工作階段，再次嘗試觸發指令。
...
**症狀**：執行 `dotnet test` 時出現 `AutoFixture` 相關例外，訊息包含 `Ploeh.AutoFixture.ObjectCreationException` 或循環參考相關說明。
...
```bash
dotnet list package samples/unit/practice/tests/Practice.Core.Tests/
```
...
Orchestrator 呈現完整結果後等待使用者決定是否啟動修改流程。若需套用 Reviewer 建議，告知 Orchestrator 後會自動進入三階段修改流程（Writer 修改 → Executor 重新執行 → Reviewer 重新審查）。
