...
Analyzer 在推導測試情境時，會把被測類別**在原始碼中明確宣告的每一個 public 建構子**都列成情境——包含只委派給另一個建構子的無參數建構子（如 `OrderValidator() : this(TimeProvider.System)`）。有 `?? throw new ArgumentNullException` 防禦的參數，另外各產生一個防禦情境。Writer 不得略過這些情境，Reviewer 會逐一對帳。

先前的規則只涵蓋**有 null guard 的參數**，於是沒有 null guard 的 public 建構子不受任何約束，寫不寫全憑當輪判斷。實測中，同一個驗證器類別的 5 次獨立執行，建構子測試數**全部為 0**；補上這條規則後，該類別的 line coverage 由 88.57% 提升到 97.14%，唯一未覆蓋的行是被 `.When()` 短路、實際上不可覆蓋的分支。

兩種情況不列管：類別沒有任何 public 建構子（`static` 類別，或建構子皆非 public）；類別在原始碼中未宣告任何建構子（只有編譯器產生的隱含無參數建構子）。後者的建構子沒有對應的原始碼行，任何建立實例的測試都已經走過它，另寫一個只是噪音。
